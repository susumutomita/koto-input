...
- 入力メソッド本体: Swift / SwiftPM（InputMethodKit + FoundationModels、`KotoInput/` 配下）
- 開発ツールランタイム/パッケージマネージャー: Bun
- リンター/フォーマッター: Biome（設定は `biome.json`）、ドキュメントは textlint
- テスト: `swift test --package-path KotoInput`（toolchain の無い環境では CI の swift ジョブが代行）
...
- **BDD スタイル**: テストのタイトル（`@Test` / `@Suite`、TS の場合は `describe`/`it`）を日本語で記述し、振る舞いを表現する。
- **No Mock**: 実際の DB・API・ファイル I/O を使う。モックデータ・スタブ API 禁止（Swift テストの例外は [ADR-0004](./docs/adr/0004-swift-テストでの-scripted-provider.md)）。
- **一気通貫**: 新機能は状態機械・プロバイダ・入力メソッド統合・テストをまとめて実装する。
...
タスク完了前に全て Green にする。`make before-commit` が harness・textlint・biome を順に実行する。

```bash
nr lint          # biome check
nr lint:text     # textlint（README）
