...
```bash
make install      # 開発ツールの依存関係インストール（Bun、--ignore-scripts）
make swift-build  # 入力メソッド本体のビルド（要 Swift toolchain / macOS）
```
...
make before-commit              # architecture-harness + textlint + biome
make swift-test                 # Swift テスト（toolchain の無い環境では CI の swift ジョブが代行）
/review                         # コードレビュー
...
| コマンド | 用途 |
| --- | --- |
| `make install` | 開発ツールの依存関係インストール |
| `make swift-build` | Swift 全ターゲットのビルド |
| `make swift-test` | Swift テスト実行 |
| `make ime-build` / `make ime-install` | Koto.app の組み立て / インストール |
| `nr lint` / `nr lint:text` | biome / textlint |
